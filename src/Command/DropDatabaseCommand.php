...
class DropDatabaseCommand extends DoctrineCommand
{
	...
	{
		$this->setName('doctrine:database:drop')
			...
			->setHelp('The <info>%command.name%</info> command drops the default connections database:
		...
		if ($input->getOption('force')) {
			...
			try {
				if ($shouldDropDatabase) {
					...
					$output->writeln(
						sprintf(
							'<info>Database for connection named <comment>%s</comment> doesn\'t exist. Skipped.</info>',
							$name
						)
					);
				}
